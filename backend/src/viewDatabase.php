...
$sql = 'SELECT o.id, o.title, o.description, o.keywords FROM objects o ORDER BY o.fullTitle ASC ';
$stmt = $db->prepare($sql);
$stmt->execute();
...
foreach (getObjects('', true) as $object) {   
    ...
    echo '<td><img width = "32" title = "' . $object['icon'] . '" src = "../../frontend/resources/images/icons/' . $object['icon'] . '" /></td><td><a href = "editor.php?objectId=' . $object['id'] . '" />' . $object['title'] . '</a>';
